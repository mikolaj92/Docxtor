...
def test_typed_paragraph_and_run_locators_cover_document_stories(tmp_path: Path) -> None:
    ...
    body.add_run("Body ")
    body.add_run("value")
    source.add_paragraph()  # Empty paragraphs retain their global coordinate.
    ...
    assert [item.paragraph_index for item in resolutions] == [0, 1, 2, 3, 4]
    ...
    assert body_resolution is not None
...
def test_typed_locator_resolution_fails_closed_for_unknown_coordinates(tmp_path: Path) -> None:
    path = tmp_path / "typed-locator-errors.docx"
    source = PyDocxDocument()
    source.add_paragraph("Known")
    source.save(path)
    document = DocxDocument.open(path)
    body = ParagraphLocator("body:p:0")

    assert document.resolve_paragraph_locator(ParagraphLocator("body:p:999")) is None
    assert document.resolve_run_locator(RunLocator(body, -1)) is None
    assert document.resolve_run_locator(RunLocator(body, 999)) is None
    assert document.resolve_run_locator(RunLocator(ParagraphLocator("footer:99:p:0"), 0)) is None
...
def test_partial_replacement_fails_closed_when_no_runs_are_affected(
    ...
    doc = DocxDocument.open(path)
    target = doc.segments[0].container_id
    monkeypatch.setattr("docxtor.docx_units._writable_units", lambda paragraph: [])
